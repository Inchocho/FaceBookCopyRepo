...
public class PostDao {
	...
	public List<PostDto> selectPostList() throws Exception{
		...
		try {
			...
			sql += "SELECT A.POST_NO AS POST_NO , (ROW_NUMBER() OVER(ORDER BY A.POST_NUM)) AS POST_NUM , A.POST_TITLE AS POST_TITLE , B.USER_NICKNAME AS USER_NICKNAME , A.POST_CREATEDATE AS POST_CREATEDATE";
			sql += ", A.POST_COUNT AS POST_COUNT";
			sql += " FROM POST_INFO_TB A, USER_INFO_TB B";
			sql += " WHERE A.POST_NO = B.USER_NO";
			sql += " ORDER BY POST_NUM ASC";
